...
public class GenericTreeEditorWindow : EditorWindow {
	...
	public static void InitGenericEditorWindow () {
		var currentWindow = (GenericTreeEditorWindow)EditorWindow.GetWindow <GenericTreeEditorWindow> ("Generic Tree", true);
...
//		BeginWindows ();
		if (genericWindow == null) {
			...
			if (GUILayout.Button ("Create Tree")) {
				...
				genericWindow = Activator.CreateInstance(type);
				(genericWindow as IGenericTreeWindow).InitGUI (treeName);
			}
			GUI.enabled = true;
			return;
		}
