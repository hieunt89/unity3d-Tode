...
public class InputManager : MonoBehaviour {
	...
	void OnEnable(){
		Pools.sharedInstance.pool.SetCurrentSelected (null);
	...
	void HandleFingerTap (LeanFinger fg){
		...
		if (Physics.Raycast (ray, out hitInfo)) {
			...
			if(e != null && e.isInteractable && e != Pools.sharedInstance.pool.currentSelected.e){
				if (debug) {
					Debug.Log ("hit entity " + e.id.value);
				}
				Pools.sharedInstance.pool.ReplaceCurrentSelected (e);
